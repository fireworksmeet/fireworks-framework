 ...
 * 孤儿文件清理自动装配。
 * <p>
 * 提供基于 Redis ZSet 的 {@link PendingFileRegistry}（默认实现 {@link RedisPendingFileRegistry}）、
 ...
 * <p>
 * 若需替换待确认注册表实现（如使用独立的 Redis 键、换用其他存储），实现 {@link PendingFileRegistry} 后
...
public class OrphanFileAutoConfiguration {
    ...
    @ConditionalOnMissingBean(PendingFileRegistry.class)
    public PendingFileRegistry pendingFileRegistry(OrphanCleanupProperties properties) {
        return new RedisPendingFileRegistry(properties);
    }

    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnBean(PendingFileRegistry.class)
    public OrphanFileGuard orphanFileGuard(PendingFileRegistry registry) {
        return new OrphanFileGuard(registry);
    }

    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnBean({StorageService.class, PendingFileRegistry.class})
    public OrphanFileCleaner orphanFileCleaner(PendingFileRegistry registry, StorageService storageService,
