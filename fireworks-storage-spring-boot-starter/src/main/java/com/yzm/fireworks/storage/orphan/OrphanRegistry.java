 ...
 * </ul>
 * 基础框架默认提供基于 Redis ZSet 的实现 {@link RedisOrphanRegistry}，天然支持高并发与多实例部署。
 * 如需替换（如换用独立 Redis 键、其他存储），实现本接口并声明为 @Bean 即可自动取代默认实现。
 */
public interface OrphanRegistry {
     ...
     * 批量确认包含不同桶的多条待确认文件记录。
     *
     * @param orphanFiles 待确认文件列表
     */
    void confirmFiles(List<OrphanFile> orphanFiles);
     ...
     */
    List<OrphanFile> listExpired(int limit);
