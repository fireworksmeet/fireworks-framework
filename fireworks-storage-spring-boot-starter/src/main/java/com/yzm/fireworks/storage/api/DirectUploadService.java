...
public interface DirectUploadService {
     ...
     */
    UploadCredential getUploadCredential(String bucket, String objectKey, Duration duration);

    /**
     * 获取表单直传（PostPolicy）凭证：前端使用返回的 url + formData 以 multipart/form-data 形式 POST 上传文件。
     * <p>
     * {@code objectKey} 为后端生成的完整对象路径（含文件名），会被精确写入 formData 的 {@code key} 字段，
     * 云厂商（OSS/S3）会直接以该值作为存储路径并忽略前端文件原名。因此在签发时即可 100% 确定
     * {@link UploadCredential#getObjectKey()} 与 {@link UploadCredential#getDisplayUrl()} 并精确返回，
     * 前端无需再做任何文件名替换。
     * <p>
     ...
     *
     * @param bucket    桶名
     * @param objectKey 后端生成的完整对象路径（含文件名），如 {@code temp/avatar/202608/a1b2c3d4e5.jpg}
     * @param duration  凭证有效期
     */
    default UploadCredential getUploadCredentialByPostPolicy(String bucket, String objectKey, Duration duration) {
