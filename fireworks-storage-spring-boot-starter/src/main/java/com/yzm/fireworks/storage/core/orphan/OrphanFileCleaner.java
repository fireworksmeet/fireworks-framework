 ...
 * 孤儿文件清理器。
 * <p>
 * 扫描 {@link PendingFileRegistry} 中已过期且未被业务确认的记录，调用 {@link StorageService#deleteFile}
 ...
 * 且底层 {@link PendingFileRegistry} 基于 Redis ZSet，天然支持分布式。
 * <p>
 * 清理策略：
 * <ul>
...
public class OrphanFileCleaner {
    ...
    public OrphanFileCleaner(PendingFileRegistry registry, StorageService storageService,
    ...
    public int cleanExpired() {
        ...
        List<PendingFile> expired = registry.listExpired(properties.getBatchSize());
        if (expired.isEmpty()) {
            return 0;
        }
        ...
        for (Map.Entry<String, List<String>> entry : bucketGroup.entrySet()) {
            ...
            try {
                ...
                storageService.deleteFile(bucket, objects);
                registry.removeAll(bucket, objects);
